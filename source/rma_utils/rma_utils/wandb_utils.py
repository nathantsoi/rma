...
class WandbSummaryWriter(SummaryWriter):
    ...
    def __init__(self, log_dir: str, flush_secs: int, cfg):
        ...
        # FIX: Define "step" as the x-axis for all metrics so wandb never
        # auto-increments its internal step counter on bare wandb.log() calls.
        ...
        # FIX: Log run metadata into wandb.config instead of wandb.log() so it
        # doesn't consume a step and cause the step=0 ordering warning.
        wandb.config.update({"log_dir": run_name})

        # FIX: Buffer all scalars for a given step and flush in one wandb.log()
        # call. This prevents fragmented rows and dropped data points.
        self._scalar_buffer: dict = {}
    ...
    def add_scalar(self, tag, scalar_value, global_step=None, walltime=None, new_style=False):
        ...
        # FIX: If we've moved to a new step, flush the previous buffer first.
        if global_step != self._current_step:
            self._flush_scalar_buffer()
            self._current_step = global_step

        # Accumulate into buffer — flushed at end of iteration via flush_scalars()
        self._scalar_buffer[self._map_path(tag)] = scalar_value

    def flush_scalars(self):
        """Call this once per training iteration to commit all buffered scalars."""
        self._flush_scalar_buffer()

    def _flush_scalar_buffer(self):
        """Flush buffered scalars to wandb in a single log call."""
        if self._scalar_buffer and self._current_step is not None:
            wandb.log({"step": self._current_step, **self._scalar_buffer})
            self._scalar_buffer = {}
    ...
    def callback(self, step):
        # FIX: Flush the scalar buffer before checking for videos so all
        # metrics for this iteration are committed together.
        self.flush_scalars()
        self.log_video_files()

    def stop(self):
        # Flush any remaining buffered scalars before finishing.
        self._flush_scalar_buffer()
        wandb.finish()
