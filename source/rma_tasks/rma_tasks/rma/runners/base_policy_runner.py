...
class BasePolicyRunner:
    ...
    def _construct_algorithm(self, obs) -> PPO:
        ...
        base_policy_class = eval(self.policy_cfg.pop("class_name"))
        base_policy: BasePolicy = base_policy_class(
            obs, self.cfg["obs_groups"], self.env.num_actions, **self.policy_cfg
        ).to(self.device)

        # initialize the algorithm
        alg_class = eval(self.alg_cfg.pop("class_name"))
        alg: PPO = alg_class(base_policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg)
